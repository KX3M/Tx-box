...
ADMINS = list(map(int, os.environ.get('ADMINS', '1562465522').split()))
if not ADMINS:
    logging.error("ADMINS variable is missing! Exiting now")
    exit(1)
    
api_id = os.environ.get('TELEGRAM_API', '27223925')
if not api_id:
    logging.error("TELEGRAM_API variable is missing! Exiting now")
    exit(1)

api_hash = os.environ.get('TELEGRAM_HASH', '01ee5a0592773d414c039fd807496eaf')
if not api_hash:
...
if not bot_token:
    ...
    exit(1)
dump_id = os.environ.get('DUMP_CHAT_ID', '-1002452266669')
if not dump_id:
...
fsub_id = os.environ.get('FSUB_ID', '-1001947593874')
if not fsub_id:
...
async def start_command(client, message):
    ...
    if verify_status["is_verified"]:
        ...
        developer_button = InlineKeyboardButton("Developer ⚡️", url="https://t.me/+ZnP1VQFy-CFmZWQ1")
